# Code based on the works of James McCauley
# Implementation of Weighted Least Contection Algorithm for SDN Load Balancer
# Authors: Jakub Kostecki, Rafal Mycek, Szymon Krzyworzeka
...
class iplb (object):
  ...
  def _do_expire (self):
      t = time.time()
      expired_keys = [k for k, v in self.memory.items() if v.is_expired]
      for key in expired_keys:
        server = self.memory[key].server
        if server in self.server_connections:
          self.server_connections[server] = max(0, self.server_connections[server] - 1)
        del self.memory[key]

      self.outstanding_probes = {k: v for k, v in self.outstanding_probes.items() if t <= v}
  ...
  def _pick_server (self, key, inport):
      """
      Wybór serwera na podstawie Weighted Least Connections
      """
